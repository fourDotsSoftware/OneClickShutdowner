...
;copy translations end
  ...
  !define MUI_FILE "OneClickShutdowner"
  !define MUI_VERSION ""
  !define MUI_PRODUCT "One Click Shutdowner"
  !define PRODUCT_SHORTCUT "One Click Shutdowner"
  !define PRODUCT_VERSION "1.0"
  
  !define MUI_ICON "exit.ico"
...
;  !define MUI_FINISHPAGE_SHOWREADME "$INSTDIR\readme.txt"
  ...
  Name "One Click Shutdowner"
  OutFile "OneClickShutdownerSetup.exe"

  InstallDir "$PROGRAMFILES\softpcapps Software\${PRODUCT_SHORTCUT}"

  InstallDirRegKey HKLM "Software\softpcapps Software\One Click Shutdowner" ""
...
VIAddVersionKey /LANG=0 "LegalCopyright" "Copyright 2024 softpcapps Software"
...
;create start-menu items
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT}\Hibernate.lnk" "$INSTDIR\Hibernate.exe" "" "$INSTDIR\Hibernate.exe" 0
  
  CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT}\Lock.lnk" "$INSTDIR\Lock.exe" "" "$INSTDIR\Lock.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT}\Logoff.lnk" "$INSTDIR\Logoff.exe" "" "$INSTDIR\Logoff.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT}\Restart.lnk" "$INSTDIR\Restart.exe" "" "$INSTDIR\Restart.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT}\Shutdown.lnk" "$INSTDIR\Shutdown.exe" "" "$INSTDIR\Shutdown.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT}\Sleep.lnk" "$INSTDIR\Sleep.exe" "" "$INSTDIR\Sleep.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_SHORTCUT}\TurnOffMonitor.lnk" "$INSTDIR\TurnOffMonitor.exe" "" "$INSTDIR\TurnOffMonitor.exe" 0      
...
;create desktop shortcut
  ...
  CreateShortCut "$DESKTOP\Hibernate.lnk" "$INSTDIR\Hibernate.exe" ""
  CreateShortCut "$DESKTOP\Lock.lnk" "$INSTDIR\Lock.exe" ""
  CreateShortCut "$DESKTOP\Logoff.lnk" "$INSTDIR\Logoff.exe" ""
  CreateShortCut "$DESKTOP\Restart.lnk" "$INSTDIR\Restart.exe" ""
  CreateShortCut "$DESKTOP\Shutdown.lnk" "$INSTDIR\Shutdown.exe" ""
  CreateShortCut "$DESKTOP\Sleep.lnk" "$INSTDIR\Sleep.exe" ""
  CreateShortCut "$DESKTOP\TurnOffMonitor.lnk" "$INSTDIR\TurnOffMonitor.exe" ""   
...
;Add files
 ...
 Call IsDotNetInstalledAdv
  ...
  File "signedn\Hibernate.exe"
  File "signedn\Lock.exe"
  File "signedn\Logoff.exe"
  File "signedn\Restart.exe"
  File "signedn\Shutdown.exe"
  File "signedn\Sleep.exe"
  File "signedn\TurnOffMonitor.exe"         
  
  File "license_agreement.rtf"                      

  
;  File /oname=4dotsAdminActions.exe  "C:\code\Misc\4dotsLanguageDownloader\bin\Debug\signedn\4dotsAdminActions.exe"             
...
;begin new uninstall strings
 ...
 ;Store installation folder
  WriteRegStr HKLM "Software\softpcapps Software\One Click Shutdowner" "" $INSTDIR
  WriteRegStr HKLM "Software\softpcapps Software\One Click Shutdowner" "InstallationDirectory" $INSTDIR
...
;Delete Start Menu Shortcuts
  Delete "$DESKTOP\Hibernate.lnk"
  Delete "$DESKTOP\Lock.lnk"
  Delete "$DESKTOP\Logoff.lnk"
  Delete "$DESKTOP\Restart.lnk"
  Delete "$DESKTOP\Shutdown.lnk"
  Delete "$DESKTOP\Sleep.lnk"
  Delete "$DESKTOP\TurnOffMonitor.lnk"
...
;Delete Uninstaller And Unistall Registry Entries
  ...
  DeleteRegKey HKLM "Software\softpcapps Software\One Click Shutdowner"
  DeleteRegKey HKCU "Software\softpcapps Software\One Click Shutdowner"
...
Function .onInstSuccess
 ...
 ExecShell "" "https://softpcapps.com/one-click-shutdowner/how-to-use.php?afterinstall=true&version=${PRODUCT_VERSION}";
...
Function OpenWebpageFunction
  ExecShell "" "https://softpcapps.com/one-click-shutdowner/how-to-use.php?afterinstall=true"
FunctionEnd
